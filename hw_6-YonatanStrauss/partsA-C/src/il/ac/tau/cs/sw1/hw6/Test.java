...
//				"the value of p1 with x=3 is 0.0");
		...
		double[] coefficients = new double[]{-4.0, -4.0,1.0};
		Polynomial p2 = new Polynomial(coefficients); // p2 = 1.0+2.0*x+3.0*x^2
		
		
		double[] coefficients1 = new double[]{0.0,3.3,0.0,7.9,0.0,0.0,9.7};
		Polynomial p3 = new Polynomial(coefficients1); // p2 = 1.0+2.0*x+3.0*x^2
		
		System.out.println(p2.degree);
		System.out.println(p2.getFirstDerivation().computePolynomial(2));

		System.out.println(Arrays.toString(p2.getFirstDerivation().coefficients));
